...
$(document).ready(function () {
    // --- Activar ítem del sidebar ---
     ...
     if (typeof AUTH_ENABLED !== 'undefined') {
        $('#enable_ad_admin_auth').prop('checked', AUTH_ENABLED);
    }
    ...
    $('#save-config-btn').on('click', function () {
        ...
        const enableAuth = $('#enable_ad_admin_auth').is(':checked'); // ← Estado actual
        ...
        // Mostrar valor del grupo
        $('#confirm-group-value').text(adGroup);
        // Sincronizar estado en el modal
        $('#confirm-auth-status').html(enableAuth ? 
            '<span class="badge badge-success">Habilitado</span>' : 
            '<span class="badge badge-danger">Deshabilitado</span>'
        );

        // ← Aseguramos que el checkbox del modal no afecte nada
        // (no usamos checkbox, solo mostramos estado)
    ...
    $('#confirm-save-btn').on('click', function () {
        const adGroup = $('#AD_GROUP').val().trim();
        // ← Usamos el estado actual del interruptor principal
        ...
        $.post('', {
            ...
            if (data.status === 'success') {
                ...
                // ← El interruptor ya está bien, pero aseguramos la variable global
