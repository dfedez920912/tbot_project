...
def users_view(request):
    ...
    if request.method == 'POST':
        ...
        if not action:
            ...
            enable_auth = 'enable_ad_admin_auth' in request.POST

            if ad_group_val:
                set_key(env_path, 'AD_GROUP', ad_group_val)
            set_key(env_path, 'AD_ADMIN_GROUP_AUTH', 'true' if enable_auth else 'false')
            ...
            try:
                ...
                for user_data in selected_users:
                    ...
                    # ← Usa get_or_create con el username exacto, sin iexact
                    user, created = User.objects.get_or_create(username=username)

                    # ← Si no fue creado, asegúrate de que el username esté bien
                    if not created:
                        if not user.username:  # Si está vacío, fíjalo
                            user.username = username

                    # ← Establecer contraseña no utilizable
    ...
    auth_enabled = is_ad_admin_group_enabled()
